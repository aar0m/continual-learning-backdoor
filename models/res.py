...
class PreTrainedRes(ContinualLearner, MemoryBuffer):
    ...
    def __init__(self, image_size, image_channels, classes,
                 ...
                 # -fc-layers
                 fc_layers=3, fc_units=100, fc_drop=0, fc_bn=True, fc_nl="relu", fc_gated=False,
        ...
        self.convE = ConvLayers(
            conv_type="resNet", block_type="basic", num_blocks=0, image_channels=image_channels,
            depth=0, start_channels=start_channels, reducing_layers=reducing_layers, batch_norm=conv_bn, nl=conv_nl,
            global_pooling=False, gated=conv_gated, output="none",
        )

        # Load pretrained ResNet18 model
        self.resnet = resnet18(weights='DEFAULT')
        # print(f"MODEL: {self.resnet}")
        
        # Remove last layer !!! Breaks code because model requires at least 1 fc layer
        ...
        # (f"\n\n\nCONVOUT UNITS: {self.conv_out_units}\n\n\n")

        # self.fcE = MLP(input_size=self.conv_out_units, output_size=100, layers=1,
        #                hid_size=[100], drop=fc_drop, batch_norm=fc_bn, nl=fc_nl, bias=bias,
        #                excitability=excitability, excit_buffer=excit_buffer, gated=fc_gated, phantom=phantom)

        self.fcE = fc_layer(512, 100, excit_buffer=True, nl='relu', drop=fc_drop,
                                    phantom=phantom)

        # mlp_output_size = fc_units if fc_layers>1 else self.conv_out_units
        mlp_output_size = self.conv_out_units
        
        self.classifier = fc_layer(mlp_output_size, classes, excit_buffer=True, nl='none', drop=fc_drop,
                                    phantom=phantom)

        # Flags whether parts of the network are frozen
        self.resnet.frozen = True
        self.convE.frozen = True
        self.fcE.frozen = False
        
        # Modify the final fully connected layer to match the number of classes
        # self.resnet.fc = fc_layer(self.resnet.fc.in_features, classes, nl='none', drop=0, phantom=False)
        # self.resnet.fc = torch.nn.Linear(self.resnet.fc.in_features, classes)
    ...
    def forward(self, x, return_intermediate=False):
        ...
        intermediate = flatten_x
        out = self.classifier(flatten_x)
    ...
    def classify(self, x, allowed_classes=None, no_prototypes=False):
        ...
        if self.prototypes and not no_prototypes:
            ...
            image_features = self.flatten(self.resnet(x))
            # print(f"\n\n\nIMAGE FEATURES: {image_features.shape}\n\n\n")
            # hE = self.fcE(image_features)
            # scores = self.classifier(hE)
            scores = self.classifier(image_features)
